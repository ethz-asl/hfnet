...
class Cmu(BaseDataset):
    ...
    def _init_dataset(self, **config):
        ...
        if config['image_names'] is not None:
            paths = [Path(base_path, config['prefix'], n)
                     for n in config['image_names']]
            base_path = Path(base_path, config['prefix'])
        else:
            search = []
            for seq in config['sequences']:
                p = Path(base_path, seq)
                if config['load_db']:
                    search.append(Path(p, 'database'))
                if config['load_queries']:
                    search.append(Path(p, 'query'))
            assert len(search) > 0
