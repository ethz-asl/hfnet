...
def adapt_homography_to_resizing(H, old_shape, new_shape):
    s = tf.to_float(new_shape)[:2] / tf.to_float(old_shape)[:2]
    mult1 = tf.diag(tf.concat([s[::-1], [1.]], 0))
    mult2 = tf.diag(tf.concat([1/s[::-1], [1.]], 0))
    H = flat2mat(tf.expand_dims(H, 0))[0]
    H = tf.matmul(mult1, tf.matmul(tf.to_float(H), mult2))
    return mat2flat(tf.expand_dims(H, 0))[0]
