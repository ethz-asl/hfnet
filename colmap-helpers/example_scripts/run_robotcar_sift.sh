...
python colmap-helpers/robotcar_db_png_to_jpg.py --db_file overcast-reference.db

# Update the provided SIFT DB file with the correct intrinsics.
python colmap-helpers/update_db_with_nvm_intrinsics.py --database_file overcast-reference.db --nvm_file all.nvm

# Use the provided NVM file for ground-truth poses of all the cameras.
mkdir initial_model_sift_robotcar
python colmap-helpers/colmap_model_from_nvm.py --database_file overcast-reference.db --nvm_file all.nvm --output_dir initial_model_sift_robotcar/
